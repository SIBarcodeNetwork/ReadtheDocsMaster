...
# Pull and add taxonomy to Blast+ output file
Load the resulting .tsv file from the BLAST+ program, add the column headers back in, and pull the NCBI taxonomy using the "getTaxonomy" function:
...
blastResults <- read_tsv('P141512SBlastResults2.tsv',
...
blastTaxonomy <- as.data.frame(getTaxonomy(unique(blastResults$staxid),'accessionTaxaApril2022.sql'))

# Add staxid column back into taxonomy results, as the "getTaxonomy" function was not written to do that

blastTaxonomy$staxid <- unique(blastResults$staxid)
...
blastResults_matched <- blastResults %>% 
...
blastResults_matched_edited <- blastResults_matched %>%
    filter(evalue < 0.01) %>%
    group_by(qseqid)

blastResults_matched_edited2 <- sort(blastResults_matched_edited$evalue,decreasing = FALSE) %>%
    slice_head(., n = 10)

# inspect results

View(blastResults_matched_edited)
...
write.csv(blastResults_matched,'P141512SBlastResults.csv', row.names = FALSE) 
```
